import { useContext, useState } from "react";
import {
  Box,
  Text,
  Center,
  FlatList,
  Flex,
  Image,
  HStack,
  Input,
  Icon,
  NativeBaseProvider
} from "native-base";
import colors from "../component/theme";
import { ThemeContext } from "../component/themeContext";
import { useNavigation } from "@react-navigation/native";
import { Keyboard, TouchableOpacity, TouchableWithoutFeedback } from "react-native";
import { Ionicons } from "@expo/vector-icons";
import Messages from "../component/DataChat";

const PesanScreen = () => {
  // const theme = { mode: "dark" };
  const { theme, updateTheme } = useContext(ThemeContext);
  let activeColors = colors[theme.mode];

  const navigation = useNavigation();
  // untuk pencarian mengguanakn usestate
  const [searchText, setSearchText] = useState(""); // Tambahkan state untuk teks pencarian
  const [filteredMessages, setFilteredMessages] = useState([]); // Tambahkan state untuk hasil pencarian
  

  // tambah kode untuk ke salah satu MUA
  const handleChatPress = (MUAData) => {
    navigation.navigate("RoomChat", {
      MUAData:{
        id : MUAData.id,
        userName: MUAData.userName,
        userImg: MUAData.userImg,
      },
    });
  };

  // fungsi untuk tombol search
  const handleSearch = () => {
    const filtered = Messages.filter(
      (item) =>
        item.userName.toLowerCase().includes(searchText.toLowerCase()) ||
        item.messageText.toLowerCase().includes(searchText.toLowerCase())
    );
    setFilteredMessages(filtered);
  };

  return (
    <NativeBaseProvider>
      <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
        <Box flex={1}>
          <Box flex={1} pl={5} pr={5} backgroundColor={activeColors.primary}>
            <Center>
              <Box>
                <HStack alignItems="center" space={2} p={2} rounded="md">
                {/* inputan search */}
                  <Input
                    placeholder="Search.."
                    color={activeColors.tint}
                    placeholderTextColor={activeColors.tint}
                    size="sm"
                    w={"87%"}
                    rounded={24}
                    value={searchText} // nilai input ke state pencarian
                    onChangeText={(text) => setSearchText(text)} // Fungsi untuk mengubah state pencarian saat input berubah
                  />
                  {/* tombol search diarahkan ke fungsi handleSearch yang sudah dibuat*/}
                  <TouchableOpacity onPress={handleSearch}>
                    <Box backgroundColor={"#A01437"} p={2} rounded={12}>
                      <Icon
                        as={Ionicons}
                        name="search"
                        size={6}
                        color={"white"}
                      />
                    </Box>
                  </TouchableOpacity>
                </HStack>
              </Box>
              <FlatList
                // data yang ditampilkan
                data={filteredMessages.length > 0 ? filteredMessages : Messages}
                keyExtractor={(item) => item.id}
                renderItem={({ item }) => (
                  <TouchableOpacity
                    style={{ width: "100%" }}
                    // saat ditekan data akan ikut dibawah ke roomchat
                    onPress={() => handleChatPress(item)}
                  >
                    <Box justifyContent={"space-between"}>
                      <Flex direction="row">
                        <Box pt={4} pb={4}>
                          <Image
                            w={"60"}
                            h={"60"}
                            rounded={"35"}
                            source={item.userImg}
                            alt="ProfileUserChat"
                          />
                        </Box>

                        <Box
                          justifyContent={"center"}
                          p={"15"}
                          pl={0}
                          ml={"3"}
                          w={"100%"}
                          borderBottomWidth={"1"}
                          borderBottomColor={"#cccccc"}
                        >
                          <Flex direction="column">
                            <Box mb={"1"}>
                              <Flex direction="row">
                                <HStack space={32}>
                                  <Text
                                    fontSize={"14"}
                                    fontWeight={"bold"}
                                    color={activeColors.tint} 
                                    w={78}
                                  >
                                    {item.userName}
                                  </Text>

                                  <Box ml={-10}>
                                    <Text fontSize={12} color={activeColors.tint}>
                                      {item.messageTime}
                                    </Text>
                                  </Box>
                                </HStack>
                              </Flex>
                            </Box>
                          
                            <Text fontSize={"14"} color={activeColors.tertiary}>
                              {item.messageText.length > 40
                                ? item.messageText.slice(0, 40) + "..."
                                : item.messageText}
                            </Text>
                          </Flex>
                        </Box>
                      </Flex>
                    </Box>
                  </TouchableOpacity>
                )}
              />
            </Center>
          </Box>
        </Box>
      </TouchableWithoutFeedback>
    </NativeBaseProvider>
  );
};

export default PesanScreen;